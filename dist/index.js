  ...
  function compile2(events) {
    ...
    while (++index < events.length) {
      ...
      if (events[index][1].type === "definition") {
        if (events[index][0] === "enter") {
          body3 = push(body3, events.slice(start, index));
          start = index;
        } else {
          head2 = push(head2, events.slice(start, index + 1));
          start = index + 1;
        }
      }
    }
      ...
      if (events[index][0] === "enter" && events[index][1].type === "strikethroughSequenceTemporary" && events[index][1]._close) {
        ...
        while (open--) {
          ...
          events[index][1].end.offset - events[index][1].start.offset === events[open][1].end.offset - events[open][1].start.offset) {
            ...
            if (insideSpan2) {
              splice(nextEvents, nextEvents.length, 0, resolveAll(insideSpan2, events.slice(open + 1, index), context));
            }
            splice(nextEvents, nextEvents.length, 0, [["exit", text5, context], ["enter", events[index][1], context], ["exit", events[index][1], context], ["exit", strikethrough, context]]);
            splice(events, open - 1, index - open + 3, nextEvents);
...
var handle = zwitch("type", {
  invalid,
  unknown,
  handlers: { comment, doctype, element, raw, root, text: text4 }
});
